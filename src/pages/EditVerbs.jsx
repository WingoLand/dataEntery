...
import { fetchCategories, fetchVerbs } from "../requests/verbsReq";
import { getVerbsCash, setVerbsCash } from "../cashStorage/verbsCashStorage";
...
export default function EditVerbs() {
  ...
  const handelFetchWords = async (category, page) => {
    ...
    try {
      const data = await fetchVerbs(category, page);

      const newWords = data.wordsArray.map((word) => {
        let cachedData = getVerbsCash(word.id);
        if (cachedData) {
          return cachedData;
        }

        setVerbsCash(word.id, word); // Store in cache
  ...
  const getCategories = async () => {
    ...
    await fetchCategories().then((data) => {
      let newData = data.map((category) => category.en);
      setCategories(newData);
      newData = data.map((category) => category.ar);
      setLoading(false);
    });
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      setShowModal(false);
      handelFetchWords(chosenCategory.en, page);
    } catch (error) {
      alert(error.message);
    }
  };
  ...
  const handleUpdate = async () => {
    ...
    const uploadedData = {
      ...
      categoryInArabic: arabicCategory.trim() || "",
  ...
  return (
    <Container className="mt-4 mb-4 text-center">
      <h2 className="fw-bold text-primary mb-4">Edit Verbs</h2>
      ...
      {loading ? (
        ...
        <Alert variant="info">No verbs available.</Alert>
      ) : (
        ...
        <Modal.Body>
          {selectedWord && (
            <>
              ...
              {/* Arabic Category */}
              <Form.Group>
                <Form.Label className="fw-semibold">Arabic Category</Form.Label>
                <div className="d-flex align-items-center">
                  <Form.Control
                    type="text"
                    disabled
                    ...
                    readOnly={!editableFields.arabicCategory}
                  />
                  {/* <i
                    className={`ms-2 bi ${
